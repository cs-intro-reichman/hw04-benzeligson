    ...
    public static boolean equals(char[] arr1, char[] arr2) {
       if (arr1.length == arr2.length){
            ...
            while (index < arr1.length) {
                if (charAt(arr1, index) != charAt(arr2, index)) {
                    return false;
                }
                index++;
            }
    ...
    */
    public static char[] concat(char[] arr1, char[] arr2) {
        ...
        for (int i = arr1.length; i < arr1.length + arr2.length; i++) {
            arr3[i] = charAt(arr2, i - arr1.length);
        }
        return arr3;
    }
    ...
    public static long hashCode(char[] arr) {
        ...
        for (int i = 0; i < arr.length; i++) {
            hash = hash + (int) (charAt(arr, counter) * Math.pow(7, i));
            counter--;
        }
        return hash;
    }
    ...
    public static int compareTo(String str1, String str2) {
        if (str1.length() != 0 && str2.length() != 0) {
            int maxToCheck = Math.min(str1.length(), str2.length());
            for (int i = 0; i < maxToCheck; i++) {
                int valueInASCII1 = (int)str1.charAt(i);
                int valueInASCII2 = (int)str2.charAt(i);
                if (valueInASCII1 < valueInASCII2) {
                    return -1;
                } else if (valueInASCII1 > valueInASCII2) {
                    return 1;
                }
            }
